...
- `lesson_page_id` 不为空时，检索范围限定到当前课时页。
...
- 开发环境未配置 OCR 时会生成本地识别提示。
- 生产环境必须配置 `service_type=ocr`。
...
- 已通过课时发布、课时详情、学习进度、课程问答、收藏与评价、题目辅导、知识点讲解、课程测验、章节练习、提交判分、错题本、薄弱点、学习计划、学习记录的集成测试。

      ...
      <TransitionGroup name="page-switch" tag="section" class="admin-content">
        ...
        <section v-if="active === 'adminModels'" key="adminModels" class="admin-page model-layout">
          ...
          <section class="model-content">
            ...
            <article v-if="modelTab === 'embedding'" class="panel-card form-panel"><div class="panel-head"><div><h2><Search :size="18" />重排模型 Rerank</h2><span>问答多路检索结果二次精排；留空模型名则不启用，自动回退原排序</span></div><button class="btn btn-secondary" @click="testRerankModel">测试</button></div><div class="form-grid"><label>供应商<AppSelect v-model="rerankDraft.provider" :options="rerankProviderOptions" /></label><label>模型<input v-model="rerankDraft.model_name" class="input" placeholder="qwen3-rerank（推荐，gte-rerank 将于 2026-05-30 下线）" /></label><label>精排条数 Top N<input v-model.number="rerankDraft.top_n" class="input" type="number" min="1" max="20" /></label><label>API Key<PasswordField v-model="rerankDraft.api_key" /></label><label class="wide-field">API Base<input v-model="rerankDraft.endpoint" class="input" placeholder="通义留空自动按模型选地址（qwen3-rerank 走兼容版 reranks 接口）；自定义填标准 /rerank 接口" /></label></div></article>

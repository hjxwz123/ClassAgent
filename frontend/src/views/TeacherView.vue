    ...
    <main class="teacher-main" :class="{ immersive: active === 'teacherPpt' }">
      <div v-if="active !== 'teacherPpt'" class="breadcrumb">
        ...
        <section class="page-actions">
          ...
          <Segmented v-if="active === 'teacherAnalytics'" v-model="analysisRange" :items="['本周','本月','本学期']" @change="loadAnalysis" />
...
const Segmented = defineComponent({ props: { modelValue: { type: String, required: true }, items: { type: Array as PropType<string[]>, required: true } }, emits: ["update:modelValue", "change"], setup(p, { emit: update }) { return () => h("div", { class: "segmented" }, p.items.map((item) => h("button", { class: { active: p.modelValue === item }, onClick: () => { update("update:modelValue", item); update("change"); } }, item))); } });
const ViewToggle = defineComponent({ props: { modelValue: { type: String, required: true } }, emits: ["update:modelValue"], setup(p, { emit: update }) { return () => h("div", { class: "view-toggle" }, [h("button", { class: { active: p.modelValue === "grid" }, onClick: () => update("update:modelValue", "grid") }, [h(Grid2X2, { size: 16 })]), h("button", { class: { active: p.modelValue === "list" }, onClick: () => update("update:modelValue", "list") }, [h(FileText, { size: 16 })])]); } });
...
.view-toggle, .segmented { display: inline-flex; overflow: hidden; border: 1px solid var(--color-border-default); border-radius: var(--radius-md); }
.view-toggle button, .segmented button { min-height: 32px; border: 0; background: white; color: var(--color-text-secondary); padding: 0 10px; }
.view-toggle .active, .segmented .active { background: var(--color-primary-600); color: white; }
